...
object AudioSplitter {
    ...
    fun splitBySizeToUri(
        sourceFile: File,
        targetPartBytes: Long,
        outputPfdFactory: (String) -> ParcelFileDescriptor
    ): Int {
        val extractor = MediaExtractor()
        extractor.setDataSource(sourceFile.absolutePath)
        ...
        fun openNewPart() {
            val baseName = sourceFile.nameWithoutExtension
            val fileName = "${baseName}_part${partIndex}.m4a"
            currentPfd = outputPfdFactory(fileName)
            val muxer = MediaMuxer(currentPfd!!.fileDescriptor, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4)
            currentTrack = muxer.addTrack(audioFormat)
            muxer.start()
            currentMuxer = muxer
            bytesWrittenInPart = 0L
