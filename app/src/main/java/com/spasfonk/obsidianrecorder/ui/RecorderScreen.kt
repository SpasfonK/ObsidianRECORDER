...
import com.spasfonk.obsidianrecorder.ui.theme.DangerRed
...
private val categoryFilters = listOf("Tout", "Réunion", "Note vocale", "Favori")
...
fun RecorderScreen(viewModel: RecorderViewModel) {
    ...
    var splitTargetSize by remember { mutableStateOf(25L * 1024 * 1024) }
    ...
    val treeLauncher = rememberLauncherForActivityResult(
        ...
        uri?.let {
            ...
            state.selectedItemForActions?.let { item ->
                viewModel.splitRecordingToUri(item, splitTargetSize, it)
            }
    ...
    val filteredRecordings = remember(state.recordings, state.selectedFilter, state.searchQuery, sortNewestFirst) {
        val base = state.recordings.filter { item ->
            val matchesFilter = when (state.selectedFilter) {
                "Tout" -> true
                "Réunion" -> item.category == RecordingCategory.REUNION
                "Note vocale" -> item.category == RecordingCategory.NOTE_VOCALE
                "Favori" -> item.category == RecordingCategory.FAVORI
                else -> true
            }
    ...
    if (state.transcriptionError != null) {
        LaunchedEffect(state.transcriptionError) {
            snackbarHostState.showSnackbar(state.transcriptionError!!)
        }
    }
            ...
            Column(modifier = Modifier.fillMaxSize().padding(padding)) {
                RecorderTopBar(
                    ...
                    onHeadsetClick = {
                        ...
                        val currentDevice = audioManager.getDevices(AudioManager.GET_DEVICES_OUTPUTS)
                            .firstOrNull()
                        val isSpeaker = currentDevice?.type == android.media.AudioDeviceInfo.TYPE_BUILTIN_SPEAKER
                        if (isSpeaker) {
                        ...
                        items(filteredRecordings, key = { it.file.absolutePath }) { item ->
                            RecordingCard(
                                ...
                                onSetCategory = { category ->
                                    viewModel.setRecordingCategory(item, category)
                                }
                            )
                        }
    ...
    state.selectedItemForActions?.let { item ->
        RecordingActionsSheet(
            ...
            onSplit = { targetBytes ->
                splitTargetSize = targetBytes
                treeLauncher.launch(null)
            },
            onSplitLocal = { targetBytes ->
                viewModel.splitRecording(item, targetBytes)
            },
